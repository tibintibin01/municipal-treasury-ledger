...
{
    public static class EncryptedBackupService
    {
        private static readonly byte[] Magic = Encoding.ASCII.GetBytes("MTOBACKUP1");
        ...
        public static bool IsEncryptedBackup(string filePath)
        {
            ...
            if (String.Equals(Path.GetExtension(filePath), ".mtobak", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        ...
        public static void CreateEncryptedBackup(string sourceDbPath, string encryptedBackupPath, string password)
        {
            ...
            string directory = Path.GetDirectoryName(encryptedBackupPath);
            ...
            using (FileStream output = File.Create(encryptedBackupPath))
            {
        ...
        public static void DecryptBackupToFile(string encryptedBackupPath, string destinationDbPath, string password)
        {
            ValidatePassword(password);
            byte[] fileBytes = File.ReadAllBytes(encryptedBackupPath);
            int minimumLength = Magic.Length + 4 + SaltSize + IvSize + HmacSize + 1;
            if (fileBytes.Length < minimumLength)
            {
                throw new InvalidDataException("Encrypted backup file is invalid or incomplete.");
            }
            ...
            {
                throw new InvalidDataException("This is not a Municipal Treasury encrypted backup.");
            }
            ...
            if (version != Version)
            {
                throw new InvalidDataException("Encrypted backup version is not supported.");
            }
            ...
            if (cipherLength <= 0)
            {
                throw new InvalidDataException("Encrypted backup file has no data.");
            }
            ...
            {
                throw new CryptographicException("Backup password is incorrect or the backup file was changed.");
            }
        ...
        private static void ValidatePassword(string password)
        {
            ...
            {
                throw new ArgumentException("Backup password is required.");
            }
        }
